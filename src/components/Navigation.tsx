...
export default function Navigation() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      setScrolled(window.scrollY > 50); // Change to > 20 or > 100 if you want earlier/later
    };
    ...
    <>
      <header
        className={`
          ...
          ${scrolled ? 'bg-black/70 backdrop-blur-md border-b border-accent/40' : 'bg-transparent'}
        `}
      >
          ...
          <div className="absolute top-full left-0 right-0 bg-black/80 backdrop-blur-md border-t border-accent/30">
            <div className="max-w-4xl mx-auto px-6 py-6 md:py-8">
              <div className="relative">
                ...
                />
                <button className="absolute right-0 top-1/2 -translate-y-1/2 text-accent text-2xl">
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="fixed inset-0 z-40 bg-black/95 backdrop-blur-lg flex flex-col items-center justify-center">
          <button
            onClick={() => setIsMenuOpen(false)}
            className="absolute top-8 right-8 text-accent text-5xl hover:text-white"
          >
            ✕
          </button>
